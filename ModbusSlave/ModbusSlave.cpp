/*
 Modbus over serial line - RTU Slave Arduino Library
 ...
 Modbus function 6 (write single register) added by Andras Tucsni
...
/* supported modbus function codes in an array for easy iteration. Add new function codes into this array !*/
        ...
        unsigned int regs_size) 
{
        ...
        /* check quantity of registers for block read/write*/
...
int preset_multiple_registers(unsigned char slave,
        unsigned int start_addr, unsigned char count, unsigned char *query,
        int *regs) 
{
        unsigned char function = FC_WRITE_REGS;   /* Function: Preset Multiple Registers */
        int status = 0;
        ...
        if (write_regs(start_addr, query, regs)) {
                ...
                written.start_addr=start_addr;
                written.num_regs=count;
        }
...
{
        unsigned char function = FC_READ_REGS; 	/* Function 03: Read Holding Registers */
        int packet_size = 3;
        int status;
        unsigned int i;
